...
public interface ReservationStatusRepository extends JpaRepository<ReservationStatus, Integer> {
    ...
    List<ReservationStatus> findByDateStatusBetween(LocalDateTime start, LocalDateTime end);
    
    // Trouver le dernier statut d'une réservation
    @Query("SELECT rs FROM ReservationStatus rs WHERE rs.reservation.id = :reservationId ORDER BY rs.dateStatus DESC LIMIT 1")
